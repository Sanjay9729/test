...
//   const fetchImage = async (imageFileId) => {
//     if (imageFileId) {
//       try {
//         const response = await fetch(getImagePreviewURL(imageFileId));
//         if (response.ok) {
...
//     const item = filtered[index];
//     setSelectedItem(item);
//     fetchImage(item.image_file_id); // Fetch image when a row is clicked
//     setModalOpen(true);
//   };
...
//     const header = ['Full Name', 'Email', 'Phone', 'Address', 'Image URL', 'SKU', 'Product'];
//     const csvRows = [header.join(',')];
...
//         const imageUrl = item.image_file_id
//         ? item.image_file_id.split(',').map(fileId => getImagePreviewURL(fileId.trim())).join(';') // For multiple images, join with a semicolon
//         : '';
//       const row = [
//         `"${item.full_name || ''}"`,
//         `"${item.email || ''}"`,
...
//     >
//       <IndexTable.Cell>
//       {item.image_file_id && item.image_file_id.length > 0 ? (
//         <div style={{ display: 'flex', gap: '8px', overflowX: 'auto' }}>
//           {/* Split the comma-separated string into an array */}
//           {item.image_file_id.split(',').map((fileId, fileIndex) => (
//             <img
//               key={fileIndex}
//               src={getImagePreviewURL(fileId.trim())} // Trim any extra spaces
...
//         </div>
//       ) : (
//         '—' // If no images, show a dash
//       )}
//       </IndexTable.Cell>
//       <IndexTable.Cell>{item.full_name || '-'}</IndexTable.Cell>
//       <IndexTable.Cell>{item.email || '-'}</IndexTable.Cell>
//       <IndexTable.Cell>{item.selected_product || '-'}</IndexTable.Cell>
//       <IndexTable.Cell>{item.product_sku || '-'}</IndexTable.Cell>
//       <IndexTable.Cell>{item.phone || '-'}</IndexTable.Cell>
//       <IndexTable.Cell>{item.address || '-'}</IndexTable.Cell>
//     </IndexTable.Row>
//   ));
...
//                     value={selectedItem.product_sku || ''}
//                     onChange={(val) => handleModalChange('product_sku', val)}
//                   />
...
const ViewWarranty = () => {
  ...
  const handleExport = () => {
    ...
    const header = ['Full Name', 'Email', 'Phone', 'Address', 'SKU', 'Image URL'];
    const csvRows = [header.join(',')];
    ...
    filtered.forEach(item => {
      ...
      let imageUrl = '';
      if (item.image_file_id && item.image_file_id.trim() !== '') {
        const fileIds = item.image_file_id.split(',').map(id => id.trim()).filter(id => id !== '');
        // Show only file IDs instead of full URLs to avoid CSV corruption
        imageUrl = fileIds.join(';');
      }
      const row = [
        ...
        // escapeCSV(item.selected_product || ''),    
        escapeCSV(item.product_sku || ''),         
        escapeCSV(imageUrl)                       
      ];
      csvRows.push(row.join(','));
    });
        ...
        {item.image_file_id && item.image_file_id.length > 0 ? (
          <div style={{ display: 'flex', gap: '8px', overflowX: 'auto' }}>
            {item.image_file_id.split(',').map((fileId, fileIndex) => (
              <img
                key={fileIndex}
                src={getImagePreviewURL(fileId.trim())} 
                alt={`Preview ${fileIndex + 1}`}
                style={{ width: '100px', height: 'auto', borderRadius: '4px' }}
              />
            ))}
